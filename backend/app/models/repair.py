...
class Repair(Base, TimestampMixin):
    ...
    order_number: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, index=True)
    ...
    expected_total: Mapped[Optional[float]] = mapped_column(Numeric(14, 2), nullable=True)
...
class RepairWork(Base):
    ...
    work_code: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, index=True)
    ...
    standard_hours: Mapped[Optional[float]] = mapped_column(Float, nullable=True)
    ...
    reference_payload: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
...
class RepairPart(Base):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    repair_id: Mapped[int] = mapped_column(ForeignKey("repairs.id"), nullable=False, index=True)
    article: Mapped[Optional[str]] = mapped_column(String(120), nullable=True, index=True)
...
class RepairCheck(Base, TimestampMixin):
    ...
    calculation_payload: Mapped[Optional[dict]] = mapped_column(JSON, nullable=True)
