...
export function ReviewQueuePanel({
  ...
  const pageSize = Math.max(reviewQueueLimit, 1);
  const currentPage = Math.floor(reviewQueueOffset / pageSize) + 1;
  const totalPages = Math.max(1, Math.ceil(reviewQueueTotal / pageSize));
  const hasPreviousPage = reviewQueueOffset > 0;
  const hasNextPage = reviewQueueOffset + reviewQueue.length < reviewQueueTotal;
      ...
      <Stack spacing={2}>
        ...
        <Stack direction="row" spacing={1} alignItems="center" justifyContent="space-between">
          ...
          <Stack direction="row" spacing={1}>
            <Button size="small" variant="outlined" disabled={!hasPreviousPage} onClick={onPreviousPage}>
              Назад
            </Button>
            <Button size="small" variant="outlined" disabled={!hasNextPage} onClick={onNextPage}>
              Далее
            </Button>
          </Stack>
        </Stack>
